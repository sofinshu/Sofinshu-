$files = Get-ChildItem -Path "c:\Users\Administrator\Desktop\bit\uwu-chan-saas\src\commands\v1" -Filter "*.js"

Write-Host "Found $($files.Count) files in v1 to enhance..."
...
foreach ($file in $files) {
    try {
        ...
        $content = $content -replace "\.setColor\('Random'\)", ".setColor('primary')"
        ...
        if (-not $content.Contains("ActionRowBuilder().addComponents")) {
            ...
            if($content.Contains("interaction.editReply({ embeds: [") -or $content.Contains("interaction.editReply({ embeds:[")) {
                 $content = $content -replace "interaction\.editReply\(\{ embeds: \[(.*?)\] \}\);?", "const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('auto_v1_$btnName').setLabel('🔄 Sync Live Data').setStyle(ButtonStyle.Secondary));`n            await interaction.editReply({ embeds: [`$1], components: [row] });"
            }
        }

        Set-Content -Path $file.FullName -Value $content
        Write-Host "✅ Processed v1 $($file.Name)"
    } catch {
