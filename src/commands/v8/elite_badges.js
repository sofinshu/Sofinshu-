    ...
    try {
      ...
      if (sub === 'grant') {
        if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
          const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('auto_ent_elite_badges').setLabel('Sync Enterprise Data').setStyle(ButtonStyle.Secondary));
          return await interaction.editReply({ embeds: [createErrorEmbed('You need the `Manage Server` permission to grant badges.')], components: [row] });
        }
        ...
        if (!badge) {
          const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('auto_ent_elite_badges').setLabel('Sync Enterprise Data').setStyle(ButtonStyle.Secondary));
          return await interaction.editReply({ embeds: [createErrorEmbed('Invalid badge selection.')], components: [row] });
        }
