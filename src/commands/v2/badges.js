    ...
    async execute(interaction) {
        try {
            ...
            if (!userData || !userData.staff || !userData.staff.achievements || userData.staff.achievements.length === 0) {
                ...
                return const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('auto_btn_badges').setLabel('� Sync Live Data').setStyle(ButtonStyle.Secondary));
            ...
            await const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('auto_btn_badges').setLabel('� Sync Live Data').setStyle(ButtonStyle.Secondary));
            await interaction.editReply({ embeds: [embed], components: [row] });
